...
Кандидат проходит интервью прямо в браузере — вопросы генерируются под его резюме и вакансию, ответы пишутся на видео, а сервис компьютерного зрения в реальном времени держит лицо в кадре и по 478 точкам отслеживает взгляд.

<div align="center">

<img src="docs/images/demo/candidate-interview.gif" width="82%" alt="Прохождение интервью: живая камера, детекция лица, персональные вопросы"/>

<sub>Десктоп: живой предпросмотр камеры с трекингом лица, панель проктора (камера · микрофон · лицо в кадре), персональный вопрос и таймер.</sub>

<br/><br/>

<img src="docs/images/demo/mobile-interview.gif" width="30%" alt="Интервью на телефоне"/>

<sub>То же самое на телефоне — установки не нужно, запись идёт через MediaRecorder API прямо в мобильном браузере.</sub>
...
flowchart LR
    ...
    MP --> HEAD["Поза головы<br/><sub>yaw · pitch · roll</sub>"]
...
SPA на React 18 + TypeScript, разделённое на **публичную** (кандидат, доступ по ссылке) и **защищённую** (рекрутер, авторизация) части. Ниже — пошаговый проход по обоим сценариям. Технологии: Vite, React Router, TanStack Query (кеш и ревалидация серверного состояния), Tailwind CSS + shadcn/ui на Radix, запись медиа через MediaRecorder API.

### 👤 Сценарий кандидата

**1. Лендинг.** Кандидат приходит по ссылке, видит условия (40–60 минут, нужны камера и микрофон, запись по согласию) и три шага процесса. Установка софта не требуется.

<div align="center">
<img src="docs/images/screens/landing.png" width="90%" alt="Лендинг: пройдите интервью в удобное время"/>
</div>

**2. Приветствие и согласие.** Стартовый экран интервью: количество вопросов, ожидаемое время, согласие на запись видео и обработку данных.

<div align="center">
<img src="docs/images/screens/candidate-welcome.png" width="90%" alt="Стартовый экран интервью с согласием на запись"/>
</div>

**3. Проверка оборудования.** Перед стартом система проверяет камеру, микрофон, уровень звука и наличие лица в кадре — четыре зелёные галочки, и можно начинать.

<div align="center">
<img src="docs/images/screens/candidate-equipment-check.png" width="90%" alt="Проверка оборудования: камера, микрофон, уровень звука, лицо в кадре"/>
</div>

**4. Прохождение интервью.** Текущий вопрос, таймер записи, прогресс по всем вопросам и живой предпросмотр камеры с трекингом лица. Ответ пишется прямо в браузере, при необходимости — перезапись.

<div align="center">
<img src="docs/images/screens/candidate-session.png" width="90%" alt="Сессия интервью: вопрос, таймер, детекция лица, запись ответа"/>
</div>

### 🧑‍💼 Сценарий рекрутера

**5. Дашборд вакансий.** Список вакансий с быстрой статистикой по каждой — число кандидатов и статус. Отсюда создаётся вакансия и открываются интервью.

<div align="center">
<img src="docs/images/screens/recruiter-vacancies.png" width="90%" alt="Дашборд рекрутера: список вакансий со статистикой"/>
</div>

**6. Создание вакансии.** Рекрутер описывает роль и при желании загружает PDF с описанием — на его основе LLM-сервис формирует банк вопросов.

<div align="center">
<img src="docs/images/screens/recruiter-create-vacancy.png" width="90%" alt="Форма создания вакансии с загрузкой PDF"/>
</div>

**7. Добавление кандидата.** Загружается резюме (PDF/DOC) — система парсит его через LLM, генерирует персональные вопросы под пару «вакансия × резюме» и выдаёт уникальную пригласительную ссылку.

<div align="center">
<img src="docs/images/screens/recruiter-add-candidate.png" width="90%" alt="Форма добавления кандидата с загрузкой резюме"/>
</div>

**8. Отчёт по интервью.** Итоговый скор, объяснение результата, транскрипт с оценкой каждого ответа, видеозапись с плеером и таймлайн честности по движению взгляда. Рекомендация *hire / hold / reject*.

<div align="center">
<img src="docs/images/screens/interview-detail.png" width="90%" alt="Отчёт по интервью: скор, оценки, видео, таймлайн взгляда"/>
</div>

**9. Аналитика.** Воронка найма, конверсия по этапам, распределение кандидатов по уровням и топ-результаты — чтобы быстро сравнить кандидатов и оценить процесс.

<div align="center">
<img src="docs/images/screens/analytics.png" width="90%" alt="Аналитика: воронка найма и распределение кандидатов"/>
</div>
